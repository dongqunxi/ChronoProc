...
def apply_norm(fn_stc, event, thr=95, trial_STC=False):        
    ...
    if trial_STC:
        fn_avg = subjects_dir+'/fsaverage/dSPM_ROIs/trial_%s' %(event)
        stcs = np.array(stcs)
        stc_avg = np.sum(stcs, axis=0)/stcs.shape[0]
        stc_avg.save(fn_avg, ftype='stc')   

def apply_sigSTC(fn_list, event, ref_evt, method='dSPM', prtmin=-0.3, prtmax=0.0, potmin=0.05, potmax=0.35):
    from mne import spatial_tris_connectivity, grade_to_tris
    from mne.stats import spatio_temporal_cluster_test, summarize_clusters_stc
    from scipy import stats as stats
    X1, X2 = [], []
    for fn_stc in fn_list:
        fn_rest = fn_stc[: fn_stc.rfind('evtW')] + 'evtW_%s_bc_norm_1-lh.stc' %ref_evt
        stc = mne.read_source_estimate(fn_stc)
        stc_rest = mne.read_source_estimate(fn_rest)
        stc.resample(100)
        stc_rest.resample(100)
        X1.append(stc.copy().crop(prtmin, prtmax).data)
        X2.append(stc_rest.copy().crop(potmin, potmax).data)
    ...
    fn_sig = subjects_dir+'/fsaverage/%s_ROIs/%s' %(method,event)
    stc_sig.save(fn_sig)
                    
def apply_rois(fn_stc_list, event, min_subject='fsaverage', thr=95):
    """
    ...
    for fn_stc in fnlist:
        ...
        data = stc.data
        stc.data[data<np.percentile(data, thr)] = 0
        #stc.lh_data[stc.lh_data < thr * np.max(stc.lh_data)] = 0
        #stc.rh_data[stc.rh_data < thr * np.max(stc.rh_data)] = 0
...
def apply_stand(fn_stc, event, radius=5.0, min_subject='fsaverage'):
    ...
    for fn_stc in fnlist:
        ...
        source_path = labels_path + '/%s/merge/' %event
...
def apply_merge(labels_path, evt_list):
    import glob, shutil
    mer_path = labels_path + '%s/merge/' %evt_list[0]
    reset_directory(mer_path)
    source0_path = labels_path + '/%s/ini/' %evt_list[0]
    source1_path = labels_path + '/%s/ini/' %evt_list[1]
...
def apply_sele(fn_stc_list, fn_src, event, min_dist):
    ...
    for fn_stc in fn_stc_list:
        ...
        source_path = labels_path + '/%s/standard/' %event 
        source = glob.glob(os.path.join(source_path, '*.*'))
        for filename in source:
